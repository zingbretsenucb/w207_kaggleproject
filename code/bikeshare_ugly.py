...
def define_massive_pipeline():
    categorical = ('season', 'holiday', 'workingday', )
    numerical = ('datetime', 'weather', 'temp', 'atemp', 'humidity', 'windspeed','log_casual', 'log_registered') # Datetime isn't numerical, but needs to be in the numeric branch
    pipeline = Pipeline([
        # Process cat & num separately, then join back together
        ('union', FeatureUnion([ 
            ('categorical', Pipeline([
...
def train_dev_big_model_search(pipeline, train_data, dev_data, train_label, parameters, RMSE_scorer):

    print("Performing grid search...")
    t0 = time()
    print(parameters)
    gs = GridSearchCV(pipeline, parameters, n_jobs=1, verbose=1, scoring=RMSE_scorer)
    features = [c for c in train_data.columns if c not in ['count', 'casual', 'registered']]
    gs.fit(train_data[features], train_data[train_label])
    print("Best parameters set:")
    best_param = gs.best_estimator_.get_params()
    for param_name in sorted(parameters.keys()):
