...
#NC added class to create program age variable
class AddProgAge(PipelineEstimator):
    
    def __init__(self):
        #initialize beginning of the program
        self.prog_beginning = datetime.strptime('2011-01-01 00:00:00', '%Y-%m-%d %H:%M:%S')

    def transform(self, X, y = None):
        #Create a program age variable
        # This creates a key error X['prog_age'] = X['datetime'] - self.prog_beginning
        X['prog_age'] = X.index - self.prog_beginning
        #convert the time delta to float of days
        X['prog_age'] = X['prog_age'].dt.total_seconds() / (24 * 60 * 60)
